...
define('SWIPY_BY_ERRNIO_VERSION', '1.7');
...
define('SWIPY_BY_ERRNIO_OPTION_NAME_NOTIFICATION', 'errnio_notification_status');
...
function swipy_by_errnio_deactivate() {
	...
	delete_option(SWIPY_BY_ERRNIO_OPTION_NAME_NOTIFICATION);
}
...
function swipy_by_errnio_uninstall() {
	...
	$plugins = swipy_by_errnio_multiple_plugins();
...
function swipy_by_errnio_get_first_post_image( $postID ) {
    $args = array(
    'numberposts' => 1,
    'order' => 'ASC',
...
function swipy_by_errnio_load_client_script() {
    ...
    wp_register_script($handle, $script_url, false, SWIPY_BY_ERRNIO_VERSION, true);
    ...
    if ( !empty($next_post) ) {
        ...
        $next_post_thumb = swipy_by_errnio_get_first_post_image($next_post->ID);
        if ($next_post_thumb) {
    ...
    if ( !empty($prev_post) ) {
        ...
        $prev_post_thumb = swipy_by_errnio_get_first_post_image($prev_post->ID);
        if ($prev_post_thumb) {
...
function swipy_by_errnio_add_settings_menu_option() {
    add_menu_page (
        ...
        'Swipy Settings',  //menu title
        'manage_options',   //capability
        'errnio-options-swipy',   //menu_slug
...
function swipy_by_errnio_add_settings_link_on_plugin($links, $file) {
    ...
    if ($file == $this_plugin) {
		$adminpage_url = admin_url( 'admin.php?page=errnio-options-swipy' );
...
function swipy_by_errnio_admin_notice() {
	...
	$settingsurl = admin_url( 'admin.php?page=errnio-options-swipy' );

	$shouldshow = get_option(SWIPY_BY_ERRNIO_OPTION_NAME_NOTIFICATION);

	if($hook_suffix == 'plugins.php' && $needregister && $shouldshow != 'hide'){
		$plugin_name = "Swipy";
		$message_id = "mobile-swipy-errnio";
...
function swipy_by_errnio_close_notification() {
	add_option(SWIPY_BY_ERRNIO_OPTION_NAME_NOTIFICATION, 'hide');
}

function swipy_by_errnio_multiple_plugins() {
...
function swipy_by_errnio_admin_page() {
    ...
    <div class="errnio-settings-wrap">
		...
		if (!$needregister) {
			...
			if ($tagId) {
				echo '<div id="errnioSettingsAdmin" class="errnio-settings-screen" data-tagId="'.$tagId.'" data-installName="'.SWIPY_BY_ERRNIO_INSTALLER_NAME.'">';
...
add_action('wp_ajax_swipy_by_errnio_close_notification', 'swipy_by_errnio_close_notification');